...
from database.mongodb import tambah_user, cek_user_didatabase, get_all_user
...
async def start_command(client: Bot, message: Message):
    ...
    if not await cek_user_didatabase(uid):
        await tambah_user(uid)
    text = message.text
    if len(text) > 7:
        try:
